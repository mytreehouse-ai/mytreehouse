...
const LotValuationSchema = z.object({
...
export async function GET(req: NextRequest) {
  try {
    ...
    const parsedQueryData = LotValuationSchema.safeParse(
    ...
    const scrappedPropertyTransactionForSaleAverageQuery = await sql.query(
      ...
      [VACANT_LOT_PROPERTY_TYPE_ID, FOR_SALE_LISTING_TYPE_ID, city_id, sqm],
    );
    ...
    const scrappedPropertyTransactionForRentAverageQuery = await sql.query(
      scrappedPropertyTransactionAverageTextQuery,
      [VACANT_LOT_PROPERTY_TYPE_ID, FOR_RENT_LISTING_TYPE_ID, city_id, sqm],
    );
