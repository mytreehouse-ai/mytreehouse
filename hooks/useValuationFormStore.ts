...
import z from 'zod'
import { propertyValuationFormSchema, personalDetailsFormSchema } from '@/components/property/valuation';
...
const useValuationFormStore = create<FormStore>((set) => ({
    ...
    setPropertyDetailValues: (values) => set(() => ({ propertyDetailValues: values })),
    setPersonalDetailValues: (values) => set(() => ({ personalDetailValues: values })),
}));
